...
def train(opt,Gs,Zs,reals,NoiseAmp):
    real_ = functions.read_image_decompose(opt)
    in_s = 0
    ...
    print('\nOverall images: ')
    for cur_real in reals:
        print('cur real shape ', cur_real.shape, torch.max(cur_real), torch.min(cur_real))
    ...
    while cur_scale_level < opt.stop_scale+1:
        ...
        if opt.nc_im == 1:
            ...
            for i in range(9):
                # plt.imsave('%s/in.png' %  (opt.out_), functions.convert_image_np(real), vmin=0, vmax=1)
                # plt.imsave('%s/original.png' %  (opt.out_), functions.convert_image_np(real_), vmin=0, vmax=1)
                plt.imsave('%s/real_scale_%d.png' %  (opt.outf, i), 
                            functions.convert_image_np_decompose(reals[cur_scale_level], i), vmin=0, vmax=1, cmap='gray')
        ...
        if epoch == (opt.niter-1):
            if opt.nc_im == 1:
                ...
                for i in range(9):
                    # plt.imsave('%s/in.png' %  (opt.out_), functions.convert_image_np(real), vmin=0, vmax=1)
                    # plt.imsave('%s/original.png' %  (opt.out_), functions.convert_image_np(real_), vmin=0, vmax=1)
                    plt.imsave('%s/fake_sample_%d.png' %  (opt.outf, i),
                                functions.convert_image_np_decompose(fake.detach(), i), vmin=0, vmax=1, cmap='gray')
                    plt.imsave('%s/G(z_opt)_%d.png' %  (opt.outf, i),
                                functions.convert_image_np_decompose(netG(Z_opt.detach(), z_prev).detach(), i), vmin=0, vmax=1, cmap='gray')
